...
mkYesodDispatch "YesodTest1" resourcesYesodTest1

-- This function allocates resources (such as a database connection pool),
-- performs initialization and creates a WAI application. This is also the
-- place to put your migrate statements to have automatic database
-- migrations handled by Yesod.
withYesodTest1 :: AppConfig DefaultEnv () -> Logger -> (Application -> IO ()) -> IO ()
withYesodTest1 conf logger f = do
    s <- staticSite
    ...
    Database.Persist.Base.withPool (dbconf :: Settings.PersistConfig) $ \p -> do
        ...
        let h = YesodTest1 conf logger s p
...
withDevelAppPort = toDyn $ defaultDevelApp withYesodTest1